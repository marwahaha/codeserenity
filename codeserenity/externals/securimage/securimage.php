...
class Securimage {
   ...
   * @var int
   */
  private $image_type = SI_IMAGE_PNG;
   ...
   */
  private $wordlist_file = "securimage/data/words/words.txt";
   ...
   * @var mixed
   */
  private $gd_font_file = "securimage/data/gdfonts/bubblebath.gdf";
   ...
   * @var string
   */
  private $ttf_file = "securimage/data/elephant.ttf";
   ...
   * @var string
   */
  private $multi_text_color = "#084B8A,#4B088A,#4B8A08";
   ...
   * @var string
   */
  private $arc_line_colors = "#D7DF01";
   ...
   * @var string
   */
  private $audio_path = "securimage/data/audio/";
   ...
   * @var resource
   */
  private $bgimg = "securimage/data/background.png";
  ...
  /* 
	Hack: Allow one liner customization of secure image
			by G.Cochez for Code Serenity (19/10/2009)
  */
  function __construct($settings = array()) 
  {
    // make sure a session is started (we need it!)
	if (session_id() == '') session_start();
	
	// set defaults
	$this->bgimg = EXTERNALS.$this->bgimg;
	$this->ttf_file = EXTERNALS.$this->ttf_file;
	$this->audio_path = EXTERNALS.$this->audio_path;
	$this->gd_font_file = EXTERNALS.$this->gd_font_file;
	
	// is there any settings to process?
	if (count($settings) > 0) $this->config($settings);
  }
  
  /*
	Set: Method to allow configuration in one command
		by G.Cochez for Code Serenity (19/10/2009)
  */
  ...
  {
    if(!empty($background_image) && is_readable($background_image)) $this->bgimg = $background_image;
    $this->doImage();
  }
  ...
  {
    if($this->use_transparent_text == true || !empty($this->bgimg)) {
    ...
    if(!empty($this->bgimg) && is_readable($this->bgimg)) $this->setBackground();
	else die($this->bgimg.' cannot read the damn file');
